<meta charset="utf-8"/>
<meta name="language" content="english">
<meta name="google" content="notranslate">

<title>Mutopedia Alpha: The World of MGG</title>
<meta name="author" content="Mutopedia">
<meta name="description" content="Mutants: The World of Mutants Genetics Gladiators" />
<meta name="keywords" content="Mutopedia, mutopedia, mutants, genetics, gladiators, MGG" />

<link rel="stylesheet" type="text/css" href="<?php echo CSS;?>default_style.css">

<link rel="icon" href="src/img/default/favicon.ico" />

<base href="<?php echo WEBROOT;?>" >


<meta name="apple-mobile-web-app-title" content="Mutants: The World of Mutants Genetics Gladiators">

<!-- Open Graph data -->
<meta property="og:locale" content="en_EN" />
<meta property="og:type" content="website" />
<meta property="og:title" content="Mutopedia" />
<meta property="og:description" content="Mutants: The World of Mutants Genetics Gladiators" />
<meta property="og:url" content="<?php echo URL;?>" />
<meta property="og:site_name" content="Mutopedia" />
<meta property="og:image" content="<?php echo URL.IMG;?>/default/preview.png" />

<!-- Facebook spec -->
<meta property="article:publisher" content="<?php echo FACEBOOK;?>" />

<!-- Twitter data -->
<meta property="twitter:card" content="summary_large_image" />
<meta property="twitter:site" content="@mutopedia" />
<meta property="twitter:domain" content="mutopedia" />
<meta property="twitter:creator" content="@mutopedia" />
<meta property="twitter:image" content="<?php echo URL.IMG;?>/default/preview.png" />
<meta property="twitter:title" content="Mutopedia" />
<meta property="twitter:description" content="Mutants: The World of Mutants Genetics Gladiators" />
<meta property="twitter:url" content="<?php echo TWITTER;?>" />


<!--Lib Scripts-->
<script type="text/javascript" src="<?php echo JSLIB;?>jquery-1.12.0.min.js"></script>
<script type="text/javascript" src="<?php echo JSLIB;?>jquery-ui-1.11.4.min.js"></script>

<!--Facebook Script-->
<script src="<?php echo JSLIB;?>facebook_login.js"></script>

<!--Class Scripts-->
<script type="text/javascript" src="<?php echo JSCLASS;?>app.js"></script>
<script type="text/javascript" src="<?php echo JSCLASS;?>interface.js"></script>
<script type="text/javascript" src="<?php echo JSCLASS;?>engine.js"></script>

<!--Animation Scripts-->
<script class="default-animation" type="text/javascript" src="<?php echo ANIMATIONS;?>default.js"></script>

<!--Stylesheets-->
<link class="default-stylesheet" name="default" rel="stylesheet" type="text/css" href="<?php echo CSS;?>default.css"></link>
