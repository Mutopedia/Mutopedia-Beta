...
function connectUser()
{
    ...
    FB.api('/me?fields=id,name,first_name,last_name,picture', function(response)
    {
      ...
      closePopUp('logIntoFb-box');

      if(!getCookie('AcceptationCookies') || getCookie('AcceptationCookies') != 1)
      {
        showPopUp("welcome-box");
      }

      logUser(response.id, response.first_name, response.last_name, "http://graph.facebook.com/" + response.id + "/picture");
    });
}
